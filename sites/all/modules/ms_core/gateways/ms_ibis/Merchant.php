...
function sentPost($params){
  ...
  if(curl_error($curl)){
    ...
    ms_core_log_error('ms_ibis', 'Error with CURL call. Settings: !settings CURL: !curl Result: !result',
      array('!settings' => ms_core_print_r($this), '!curl' => ms_core_print_r(curl_getinfo($curl)), '!result' => $result));
  }
  curl_close ($curl);
  return $result;
}
