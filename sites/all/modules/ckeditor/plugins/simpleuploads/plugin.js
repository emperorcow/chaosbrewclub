...
function getEditorCss( config ) {
	return '.SimpleUploadsOverEditor { ' + (config.simpleuploads_editorover || 'box-shadow: 0 0 10px 1px #999999 inset !important;') + ' }' +
		...
		'span.SimpleUploadsTmpWrapper>span { top: 50%; margin-top: -0.5em; width: 100%; text-align: center; color: #fff; ' +
		...
		'.SimpleUploadsTmpWrapper .uploadCancel, .SimpleUploadsTmpWrapper a { background-color: #333333;border-radius: 0.5em;color: #FFFFFF;cursor: pointer !important;' +
		...
		'.SimpleUploadsTmpWrapper a { width:1em; padding-left:0}';
}
...
function PickAndUploadFile(editor, forImage, caller, callback) {
		...
		filePicker.addEventListener("change", function () {
			for(var i=0; i<filePicker.files.length; i++)
			{
				var file = filePicker.files[i],
					data = CKEDITOR.tools.extend({}, filePicker.simpleUploadData);

				data.file = file;
				data.name = file.name;
				data.id = getTimeStampId();

				if (!data.callback)
					createPreview(data, filePickerEditor, filePickerForceLink);

				// Upload the file
				if (uploadFile( filePickerEditor, filePickerForceLink, data ))
				{
					if (data.element) {
						// insert a space between links
						if (i>0 && data.element.getName()=='a')
	...
	function setUrl( fileUrl, data )
	{
		...
		filePickerEditor.fire('simpleuploads.endUpload', { name: IEUpload_fileName, ok: (!!fileUrl) } );

		if (IEUpload_callback)
		{
			IEUpload_callback.upload( fileUrl );
			IEUpload_fileName = null;
		...
		if ( fileUrl )
		{
			...
			if (el.$.nodeName == 'SPAN')
			{
				...
				img.on('load', function(e) {
					...
					editor.fire('simpleuploads.finishedUpload', { name: data.name, element: img } );
...
function createSVGAnimation( file, id, editor )
{
	...
	var cancelSpan = doc.createElement('a');
	cancelSpan.setAttribute('href', '#'); // just to trick the ACF
...
if (CKEDITOR.skins)
{
	...
	function createSimpleUpload(element, editor, dialogName, definition)
	{
		...
		var callback = {
			...
			upload : function(url) {
				...
				if (definition.onFileSelect)
					definition.onFileSelect(url); // missing , data
			}
		};
	...
	function applySimpleUpload( editor, dialogName, definition, elements )
	{
			for ( var i in elements )
			{
				...
				if ( element.filebrowser.url )
					createSimpleUpload(element, editor, dialogName, definition);
			}
	}
	...
	CKEDITOR.on( 'dialogDefinition', function( evt )
	{
		...
		{
			applySimpleUpload(evt.editor, evt.data.name, definition, definition.contents[ i ].elements);
		}
 ...
 *
 *		config.simpleuploads_containerover='border:1px solid red !important;';
 *
 * @since 2.7
 ...
 *
 *		config.simpleuploads_editorover='background-color:yellow !important;';
 *
 * @since 2.7
 ...
 *
 *		config.simpleuploads_coverover='border:1px solid red !important;';
 *
 * @since 4.0
 ...
 *
 *		config.simpleuploads_dialogover='border:1px solid red !important;';
 *
 * @since 4.0
 ...
 *
 * @since 4.0
 * @cfg {String} [simpleuploads_respectDialogUploads=null]
 * @member CKEDITOR.config
 */
